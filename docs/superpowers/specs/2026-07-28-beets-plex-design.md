...
The first attempt at this design was built as a single branch of 31 commits
and roughly 6,000 lines, then reviewed. That is the wrong order. Three rounds
...
The work therefore ships as six pull requests, each closing its own issue and
...
- `scan.flush` must read config **inside** its try: it runs from `cli_exit`,
  which beets does not guard, so a malformed value would break every command.
...
- `item.try_write()` returns True for formats with no rating style (WMA,
  ...
  sync pushes that phantom clear to Plex.
